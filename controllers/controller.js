...
questions.forEach((q, index) => {
  document.querySelector("main").innerHTML += `<div class="question">
...
document.querySelectorAll(".box-input > span").forEach((span, index) => {
  span.onkeyup = (e) => {
    if (e.key == "Enter") {
      ...
      console.log(`entrada: ${span.innerHTML}`);
      try {
      ...
      if (error == null) {
        div_result.innerHTML = showDivReponse(
          span.innerHTML,
          JSON.stringify(resultado_funcao),
          "sucessul"
        );
      } else {
